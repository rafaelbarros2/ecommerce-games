...
public class UserServiceTest {
	...
	public void callingWithoutParameterShouldReturnBadRequest() throws Exception {
		...
		result.add((Stack) new User((long) 1, "name", "email", "phone", "password"));
		Mockito.when(userService.findById((long) 5)).thenReturn((User) result);
	...
	public void shouldReturnJustOneFromResult() throws Exception {
		...
		result.add((Store) new User((long) 1, "name", "email", "phone", "password"));
		Mockito.when(userService.findById((long) 1)).thenReturn((User) result);
		this.mockMvc.perform(get("/user").queryParam("1")).andExpect(MockMvcResultMatchers.status().isOk())
